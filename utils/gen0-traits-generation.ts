...
import { ZeniClasses, ZeniGenerations, ZeniImageCombo } from '../types';
import * as fs from 'fs';
import { encryptZeniARN } from './arn';
...
function generateGen0ZeniCombo(
  ...
  const classBodies = IMAGES_BY_TRAIT[ZeniImageTraits.Body].filter((imageInfo) => imageInfo.class === zeniClass);
  ...
  combo.arn = encryptZeniARN(combo);

  if (typeof items[combo.arn] === 'undefined') {
    return combo;
  } else {
    return generateGen0ZeniCombo(zeniClass, classId, items);
  }
}
...
export const logResults = (traits: { [arn: string]: ZeniImageCombo }) => {
  ...
  IMAGES_BY_TRAIT[ZeniImageTraits.Body].forEach((img) => (bodyCount[img.displayName] = { total: 0, percent: '' }));
  allImages.forEach((img) => {
    bodyCount[img.body.displayName].total += 1;
    bodyCount[img.body.displayName].percent = `${getPercentageAmongAllZenis(bodyCount[img.body.displayName].total)}%`;
  });
  ...
  IMAGES_BY_TRAIT[ZeniImageTraits.Skin].forEach((img) => (skinsCount[img.displayName] = { total: 0, percent: '' }));
  allImages.forEach((img) => {
    skinsCount[img.skin.displayName].total += 1;
    skinsCount[img.skin.displayName].percent = `${getPercentageAmongAllZenis(skinsCount[img.skin.displayName].total)}%`;
  });
  ...
  IMAGES_BY_TRAIT[ZeniImageTraits.Eyes].forEach((img) => (eyesCount[img.displayName] = { total: 0, percent: '' }));
  allImages.forEach((img) => {
  ...
  IMAGES_BY_TRAIT[ZeniImageTraits.Mouth].forEach((img) => (mouthCount[img.displayName] = { total: 0, percent: '' }));
  allImages.forEach((img) => {
  ...
  IMAGES_BY_TRAIT[ZeniImageTraits.Crown].forEach((img) => (crownsCount[img.displayName] = { total: 0, percent: '' }));
  allImages.forEach((img) => {
...
export const generateTraits = (): {
  ...
  for (let x = 0; x < TOTAL_PER_CLASS_PUBLIC; ++x) {
    const newCombo = generateGen0ZeniCombo(ZeniClasses.Magi, magiId, combosByArn);
  ...
  let rogueId = 1;
  for (let x = 0; x < TOTAL_PER_CLASS_PUBLIC; ++x) {
    const newCombo = generateGen0ZeniCombo(ZeniClasses.Rogue, rogueId, combosByArn);
    combosByArn[newCombo.arn] = newCombo;
  ...
  let warriorId = 1;
  for (let x = 0; x < TOTAL_PER_CLASS_PUBLIC; ++x) {
    const newCombo = generateGen0ZeniCombo(ZeniClasses.Warrior, warriorId, combosByArn);
    combosByArn[newCombo.arn] = newCombo;
